...
class Task1Export implements FromQuery,WithHeadings
{
    use Exportable;
    ...
    {
        $q = Subscriber::query();
        return $q->select('jos_osmembership_subscribers.*','jos_user_usergroup_map.group_id')
            ->join('jos_user_usergroup_map' ,'jos_osmembership_subscribers.user_id','jos_user_usergroup_map.user_id')
            ->whereIn('plan_id', [ 1, 5, 7, 8, 26, 27])
            ->where('published', 1);
    }
